  ...
  return (
    <div className={`p-4 rounded-2xl bg-gradient-to-br ${colorClasses[color]} border`}> 
      <div className="flex items-center justify-between mb-2">
        <Icon name={icon} size={20} className="text-slate-200" />
        {trend !== undefined && (
          <span className={`text-xs font-medium ${trend > 0 ? 'text-emerald-300' : 'text-slate-400'}`}>
            {trend > 0 ? '+' : ''}{trend}%
          </span>
        )}
      </div>
...
function QuickAction({ to, icon, label, description }) {
  return (
    <Link
      to={to}
      className="p-4 rounded-xl glass-panel hover-tilt transition-all group"
    >
      <div className="flex items-center gap-3">
        <div className="glass-panel h-10 w-10 rounded-xl flex items-center justify-center">
          <Icon name={icon} size={18} className="text-slate-200" />
        </div>
        <div>
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="glass-panel rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/admin/announcements" className="text-xs text-slate-300 hover:text-white">
              View all
            </Link>
          </div>
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-3xl font-bold text-emerald-300">{activeAnnouncements}</p>
        ...
        <div className="glass-panel rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/admin/alerts" className="text-xs text-slate-300 hover:text-white">
              View all
            </Link>
          </div>
          <div className="flex items-center gap-4">
            ...
            <div className="w-px h-10 bg-white/10" />
            <div>
              <p className="text-3xl font-bold text-amber-300">{activeAlerts}</p>
              <p className="text-xs text-slate-500">Active</p>
            </div>
          </div>
        </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {loading ? (
            <div className="p-4 rounded-xl glass-panel text-slate-400 text-sm">
              Loading...
            </div>
            ...
            announcements.slice(0, 5).map((announcement) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <span className={`w-2 h-2 rounded-full ${announcement.active ? 'bg-emerald-300' : 'bg-slate-600'}`} />
                  <div>
