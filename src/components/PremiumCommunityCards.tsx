...
import { MessageSquare, TrendingUp, Users, Eye, Building2, Briefcase, Flame, MapPin } from 'lucide-react';
import type { HomeCommunityPreview, HomeCommunityTopic } from '../services/api/types';
...
function iconForTopic(icon: HomeCommunityTopic['icon']) {
  if (icon === 'briefcase') return Briefcase;
  if (icon === 'bank') return Building2;
  if (icon === 'house') return Building2;
  if (icon === 'pin') return MapPin;
  if (icon === 'fire') return Flame;
  return MessageSquare;
}

export function PremiumCommunityCards({ townHall, hotTopics, isLive = false }: PremiumCommunityCardsProps) {
  const previews = townHall?.previews || [];
        ...
        <div className="relative overflow-hidden rounded-[24px] bg-gradient-to-b from-[#1a2642]/90 via-[#14203a]/95 to-[#0f1829]/98 backdrop-blur-xl p-5 text-left h-full">
          ...
          <div className="absolute top-4 right-4">
            <div className="relative flex items-center gap-1.5 px-2 py-1 rounded-full bg-green-500/10 backdrop-blur-sm border border-green-400/20">
              ...
              />
              <span className="text-[10px] text-green-400 font-semibold">{isLive ? 'LIVE' : 'SYNC'}</span>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <div className="space-y-2">
              {previews.slice(0, 2).map((preview) => (
                <div key={preview.id} className="flex items-center gap-2">
                  <div className="relative">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] text-white/70 line-clamp-1">{preview.title}</p>
                    <p className="text-[9px] text-white/40">{preview.timestamp}</p>
                  </div>
                  {preview.isNew ? (
                    <span className="text-[10px] text-green-400 font-semibold">NEW</span>
                  ) : (
                    <span className="text-[10px] text-white/40">{preview.replies} replies</span>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="relative">
            ...
            <div className="space-y-2">
              {hotTopics.slice(0, 3).map((topic, index) => {
                const TopicIcon = iconForTopic(topic.icon);
                return (
                  <motion.div
                    key={`${topic.id}-${index}`}
                  ...
                  >
                    ...
                    <div className="relative flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <TopicIcon className="w-3.5 h-3.5 text-white/70" strokeWidth={2} />
                        <span className="text-[11px] text-white/80 font-medium">{topic.label}</span>
                      </div>
